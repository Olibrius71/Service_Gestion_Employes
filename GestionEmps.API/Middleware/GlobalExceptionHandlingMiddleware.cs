...
using GestionEmps.Core.Models;

namespace GestionEmps.API.Middleware;
...
public class GlobalExceptionHandlingMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        var errorResponse = exception switch
        {
            ...
            ArgumentNullException => ErrorResponse.Create("Un paramètre requis est manquant.", "ARGUMENT_NULL", 400, traceId),
            ArgumentException => ErrorResponse.Create("Un paramètre fourni est invalide.", "INVALID_ARGUMENT", 400, traceId),
            UnauthorizedAccessException => ErrorResponse.Create("Accès non autorisé.", "UNAUTHORIZED", 401, traceId),
            NotImplementedException => ErrorResponse.Create("Fonctionnalité non implémentée.", "NOT_IMPLEMENTED", 501, traceId),
            TimeoutException => ErrorResponse.Create("L'opération a expiré.", "TIMEOUT", 408, traceId),
            _ => ErrorResponse.Create("Une erreur interne du serveur est survenue.", "INTERNAL_SERVER_ERROR", 500, traceId)
        };
